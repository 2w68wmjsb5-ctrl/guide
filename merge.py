...
sequence.append((dark, 2, True))    # grundlagen divider
for i in range(len(l2.pages)):
    sequence.append((l2, i, False))
sequence.append((dark, 3, True))    # phrasen divider
for i in range(len(l3.pages)):
    sequence.append((l3, i, False))
sequence.append((dark, 4, True))    # techniken divider
for i in range(len(l4.pages)):
    sequence.append((l4, i, False))
sequence.append((dark, 5, True))    # bonus divider
for i in range(len(l5.pages)):
    sequence.append((l5, i, False))
sequence.append((dark, 6, True))    # closing
...
# Build overlay stack (one small pdf per page with just the footer stamp)
...
for page_num, (src, idx, is_dark) in enumerate(sequence, start=1):
    ...
    c.drawString(margin_x + brand_w + 4, bottom_y, "·  MUAY THAI LEXIKON")
...
with open("PATTO_Muay_Thai_Lexikon_GUIDE.pdf", "wb") as f:
    writer.write(f)

print("done ->  PATTO_Muay_Thai_Lexikon_GUIDE.pdf")
